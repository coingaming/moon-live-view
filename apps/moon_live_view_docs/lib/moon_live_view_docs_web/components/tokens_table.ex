defmodule MoonLiveViewDocsWeb.TokensTable do
  @moduledoc "TokensTable"
            ...
            <table class="bg-secondary w-full table-auto text-primary">
              <thead>
                <tr class="border-b-1 border-primary">
                  <th class="bg-tertiary w-space-128 lg:w-1/3 w-space-full text-start select-none text-body-300 p-space-12 border-e-1 border-primary">
                    <span class="font-medium">Visual</span>
                  </th>
              ...
              <tbody>
                <%= for row <- @data do %>
                  <tr id={row.id} class="border-b-1 last:border-b-0 border-secondary text-primary">
                    <td class="text-start w-space-128 lg:w-1/3 max-w-md text-body-300 px-space-12 py-space-8">
                      {row.visual}
                    </td>
                    ...
                    <td
                      data-token={row.value}
                      class="text-start text-body-300 px-space-12 py-space-8 whitespace-nowrap"
                    >
                      <%= unless row.value =~ "--" do %>
                        {row.value}
                      <% end %>
